...
export async function onRequest({ request, env }) {
  ...
  // Keep the editable assessment summary intact; reject oversize enquiries explicitly.
  if (String(data.problem || "").length > 20000)
    return json({ ok: false, error: "message_too_long" }, 400);
  const problem = text(data.problem, 20000);
  const attached = Array.isArray(data.attached)
    ? data.attached
        .slice(0, 3)
        .map((s) => text(s, 600))
        .filter(Boolean)
  ...
  if (!name || !company || !problem || !EMAIL.test(email))
  ...
  const body =
    ...
    (attached.length
      ...
      : "") +
    "\n\n--\nSent from the dfg.consulting contact form. Reply goes to " +
    email +
    ".";
